...
describe("bitbucket-events-schema", () => {
    describe("bitbucketEventSchema", () => {
        const eventDataFiles = glob.sync("__test-data__/*.json", { cwd: __dirname })
    ...
    describe("pullRequestApprovalEventSchema", () => {
        it("parses reviewer approved event", async () => {
            const eventData = await import("@/bitbucket-to-gchat/events/__test-data__/pr-reviewer-approved.json")
        ...
        it("parses reviewer needs work event", async () => {
            const eventData = await import("@/bitbucket-to-gchat/events/__test-data__/pr-reviewer-needs-work.json")
    ...
    describe("pullRequestReviewersUpdatedEventSchema", () => {
        it("parses reviewer updated event", async () => {
            const eventData = await import("@/bitbucket-to-gchat/events/__test-data__/pr-reviewer-updated.json")
